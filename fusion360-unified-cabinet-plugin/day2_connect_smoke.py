#!/usr/bin/env python3
"""Day 2 Connect smoke — preview screw holes + full cut pipeline.

Run from Fusion Scripts and Add-ins (Scripts tab, ▶ play):
...
def _ensure_paths(plugin_dir: str) -> None:
    # Keep in sync with UnifiedCabinetPlugin._ensure_paths (flat imports under fusion/metadata/…).
    for path in (
        plugin_dir,
