"""Fusion smoke runner — drawer runner host-only cut + writeback."""
...
def run_drawer_runner_hole_fusion_smoke(
    plugin_dir: str,
    fusion,
    rel_ctrl,
    hw_ctrl,
    *,
    ...
    def build_result(overall: bool, **extra) -> Dict[str, Any]:
        payload = {
            ...
            "smoke": "drawer_runner_hole_connect",
            "action": "hardware.runDrawerRunnerHoleSmoke",
        ...
        if write_json:
            ...
            try:
                ...
                out_path = os.path.join(out_dir, "drawer_runner_hole_connect_smoke_results.json")
    ...
    preview_payload = {"relationship": relationship, "rule": dict(RUNNER_RULE), "panels": panels}
    _ev, preview_report = hw_ctrl.preview_drawer_runner_holes_from_relationship(preview_payload, None)
    feature = ((preview_report.get("features") or [{}])[0]) if isinstance(preview_report, dict) else {}
    preview_ok = (
        bool(preview_report.get("ok"))
        and feature.get("hostRole") == "runner_mount"
        and int(preview_report.get("holeCount") or 0) >= 2
        and bool((feature.get("geometry") or {}).get("positions"))
        and preview_report.get("cutReady") is True
    )
    if not record("3 preview runner holes", preview_ok, {
    ...
    cut_payload = {"relationship": confirmed, "rule": dict(RUNNER_RULE), "panels": panels}
    _ev, cut_report = hw_ctrl.create_drawer_runner_holes_from_relationship(cut_payload, None)
    ...
    if not record("6 host runner hole cut", cut_ok, {
