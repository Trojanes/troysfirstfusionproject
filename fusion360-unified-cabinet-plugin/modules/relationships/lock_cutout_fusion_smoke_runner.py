"""Fusion smoke runner — lock pocket host-only cut + writeback."""
...
def run_lock_cutout_fusion_smoke(
    plugin_dir: str,
    fusion,
    rel_ctrl,
    hw_ctrl,
    *,
    ...
    def build_result(overall: bool, **extra) -> Dict[str, Any]:
        payload = {
            ...
            "smoke": "lock_cutout_connect",
            "action": "hardware.runLockCutoutSmoke",
        ...
        if write_json:
            ...
            try:
                ...
                out_path = os.path.join(out_dir, "lock_cutout_connect_smoke_results.json")
    ...
    preview_payload = {"relationship": relationship, "rule": dict(LOCK_RULE), "panels": panels}
    _ev, preview_report = hw_ctrl.preview_lock_cutout_from_relationship(preview_payload, None)
    feature = ((preview_report.get("features") or [{}])[0]) if isinstance(preview_report, dict) else {}
    pocket = (feature.get("geometry") or {}).get("pocket") or {}
    preview_ok = (
        bool(preview_report.get("ok"))
        and feature.get("hostRole") == "lock_pocket"
        and float(pocket.get("depthMm") or 0) > 0
        and abs(float(pocket.get("u1") or 0) - float(pocket.get("u0") or 0)) > 0
        and abs(float(pocket.get("v1") or 0) - float(pocket.get("v0") or 0)) > 0
        and preview_report.get("cutReady") is True
    )
    if not record("3 preview lock pocket", preview_ok, {
    ...
    _ev, cut_report = hw_ctrl.create_lock_cutout_from_relationship(cut_payload, None)
    ...
    if not record("6 host lock pocket cut", cut_ok, {
